"""
Target RHUI (Red Hat Update Infrastructure) content access.

Single responsibility: on cloud systems, make the target repositories reachable
by swapping the source RHUI client for the target one inside the scratch
container, report which repoids the target clients provide, and clean up the
repofiles leapp injected to bootstrap that swap. On non-RHUI systems the public
entry points are no-ops.

This module depends only on the leaf ``layout`` module and shared leapp
libraries - nothing build-side or discovery-side - so ``targetrepos`` can call
``get_rhui_available_repoids`` without creating an import cycle.
"""
...
def remove_injected_repofiles(target_userspace_ctx, rhui_setup_info):
