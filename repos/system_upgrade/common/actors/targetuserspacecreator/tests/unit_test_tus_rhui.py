...
*observable* behavior of the current implementation so the redesign that split
the RHUI code out of ``userspacegen.py`` into its own module can be verified to
preserve it.

These functions live in ``libraries/rhui.py``; the tests import ``rhui`` (and
``layout``) directly and target their public entry points, not the surrounding
orchestration.
...
from leapp.libraries.actor import tus_layout, tus_rhui
...
def test_r1_copy_location_dst_is_dir(monkeypatch):
    ...
    assert tus_rhui.get_copy_location_from_copy_in_task('/base', task) == '/etc/yum.repos.d/foo.repo'
...
def test_r1_copy_location_dst_is_file(monkeypatch):
    ...
    assert tus_rhui.get_copy_location_from_copy_in_task('/base', task) == '/etc/yum.repos.d/bar.repo'
...
def test_r3_preinstall_tasks_remove_then_copy(monkeypatch):
    ...
    tus_rhui._apply_rhui_access_preinstall_tasks(ctx, _setup_info(preinstall=preinstall))
...
def test_r4_postinstall_tasks_copy_with_cp(monkeypatch):
    ...
    tus_rhui._apply_rhui_access_postinstall_tasks(ctx, _setup_info(postinstall=postinstall))
...
def test_r2_foreign_repofiles_hidden_and_restored(monkeypatch):
    ...
    repoids = tus_rhui.get_rhui_available_repoids(ctx, rhui_info)
...
def test_r2_restores_foreign_repofiles_on_repolist_failure(monkeypatch):
    ...
    with pytest.raises(StopActorExecutionError) as err:
        tus_rhui.get_rhui_available_repoids(ctx, rhui_info)
...
def _patch_setup_env(monkeypatch):
    monkeypatch.setattr(api, 'current_logger', logger_mocked())
    monkeypatch.setattr(api, 'current_actor', CurrentActorMocked(dst_ver='9.4'))
    monkeypatch.setattr(tus_rhui, 'get_target_major_version', lambda: '9')
    monkeypatch.setattr(tus_layout, 'target_userspace_path', lambda: '/target')
    monkeypatch.setattr(tus_layout, 'create_target_userspace_directories', lambda path: None)


def test_r5_no_rhui_info_is_noop(monkeypatch):
    _patch_setup_env(monkeypatch)
    ctx = RhuiContextMock()
    tus_rhui.setup_target_rhui_access_if_needed(ctx, _indata(None))
...
def test_r5_bootstrap_disabled_only_applies_preinstall(monkeypatch):
    ...
    tus_rhui.setup_target_rhui_access_if_needed(ctx, _indata(_rhui_info(setup_info)))
...
def test_r5_client_swap_command_and_transaction(monkeypatch):
    ...
    tus_rhui.setup_target_rhui_access_if_needed(ctx, _indata(rhui_info))
...
def test_r6_removes_unowned_repofiles_keeps_owned(monkeypatch):
    ...
    tus_rhui.remove_injected_repofiles(Ctx(), _setup_info(preinstall=preinstall))
