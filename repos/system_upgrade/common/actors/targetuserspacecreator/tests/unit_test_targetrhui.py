"""
Characterization tests for the RHUI setup path (libraries/targetrhui.py).

The RHUI client-swap path is cloud-only and historically had no unit coverage
(see the actor's #486 refactor plan, section H). These tests lock in the
*observable* behavior of the current implementation so the follow-up structural
refactor of ``setup_target_rhui_access_if_needed`` can be verified to preserve
it. They intentionally target the public entry points (not internal helpers),
so they keep working across the refactor.

The checklist items referenced below (R1-R7) come from the refactor plan's
"C-RHUI" acceptance gate.
"""
...
from leapp.libraries.actor import bootstrap, repoaccess, targetrhui
...
def test_r1_copy_location_dst_is_dir(monkeypatch):
    ...
    assert targetrhui.get_copy_location_from_copy_in_task('/base', task) == '/etc/yum.repos.d/foo.repo'
...
def test_r1_copy_location_dst_is_file(monkeypatch):
    ...
    assert targetrhui.get_copy_location_from_copy_in_task('/base', task) == '/etc/yum.repos.d/bar.repo'
...
def test_r3_preinstall_tasks_remove_then_copy(monkeypatch):
    ...
    targetrhui._apply_rhui_access_preinstall_tasks(ctx, _setup_info(preinstall=preinstall))
...
def test_r4_postinstall_tasks_copy_with_cp(monkeypatch):
    ...
    targetrhui._apply_rhui_access_postinstall_tasks(ctx, _setup_info(postinstall=postinstall))
...
def test_r2_foreign_repofiles_hidden_and_restored(monkeypatch):
    ...
    repoids = targetrhui._get_rhui_available_repoids(ctx, rhui_info)
...
def test_r2_restores_foreign_repofiles_on_repolist_failure(monkeypatch):
    ...
    with pytest.raises(StopActorExecutionError) as err:
        targetrhui._get_rhui_available_repoids(ctx, rhui_info)
...
def _patch_setup_env(monkeypatch):
    monkeypatch.setattr(api, 'current_logger', logger_mocked())
    monkeypatch.setattr(api, 'current_actor', CurrentActorMocked(dst_ver='9.4'))
    monkeypatch.setattr(targetrhui, 'get_target_major_version', lambda: '9')
    monkeypatch.setattr(bootstrap, '_get_target_userspace', lambda: '/target')
    monkeypatch.setattr(bootstrap, '_create_target_userspace_directories', lambda path: None)


def test_r5_no_rhui_info_is_noop(monkeypatch):
    _patch_setup_env(monkeypatch)
    ctx = RhuiContextMock()
    targetrhui.setup_target_rhui_access_if_needed(ctx, _indata(None))
    assert ctx.events == []
...
def test_r5_bootstrap_disabled_only_applies_preinstall(monkeypatch):
    ...
    targetrhui.setup_target_rhui_access_if_needed(ctx, _indata(_rhui_info(setup_info)))
...
def test_r5_client_swap_command_and_transaction(monkeypatch):
    ...
    targetrhui.setup_target_rhui_access_if_needed(ctx, _indata(rhui_info))
...
def test_r6_removes_unowned_repofiles_keeps_owned(monkeypatch):
    ...
    targetrhui._remove_injected_repofiles_from_our_rhui_packages(Ctx(), _setup_info(preinstall=preinstall))
