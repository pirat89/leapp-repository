...
def _consume_data():
    ...
    try:
        target_repos = next(api.consume(TargetRepositoriesFacts)).repositories
    except StopIteration:
        raise StopActorExecutionError(
            'Could not check for valid GPG keys', details={'details': 'No TargetRepositoriesFacts facts'}
        )
    try:
...
def process():
    ...
    them from model TargetRepositoriesFacts.
    """
